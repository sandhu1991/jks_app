//#region node_modules/ssr-window/dist/ssr-window.esm.js
/**
* SSR Window 2.0.0
...
//#endregion
//#region node_modules/dom7/dist/dom7.modular.js
/**
* Dom7 2.1.5
...
function each(callback) {
	...
	for (let i = 0; i < this.length; i += 1) if (callback.call(this[i], i, this[i]) === false) return this;
	return this;
}
...
"resize scroll".split(" ");
//#endregion
//#region node_modules/swiper/js/swiper.esm.bundle.js
/**
* Swiper 5.4.5
		...
		for (let i = 1; i < args.length; i += 1) {
			...
			if (nextSource !== void 0 && nextSource !== null) {
				...
				for (let nextIndex = 0, len = keysArray.length; nextIndex < len; nextIndex += 1) {
					...
					if (desc !== void 0 && desc.enumerable) if (Utils.isObject(to[nextKey]) && Utils.isObject(nextSource[nextKey])) Utils.extend(to[nextKey], nextSource[nextKey]);
...
function updateSlides() {
	...
	for (let i = 0; i < slidesLength; i += 1) {
		...
		if (params.slidesPerView === "auto") {
			...
			if (params.roundLengths) slideSize = Math.floor(slideSize);
			if (slides[i]) if (swiper.isHorizontal()) slides[i].style.width = `${slideSize}px`;
...
function updateAutoHeight(speed) {
	...
	if (swiper.params.slidesPerView !== "auto" && swiper.params.slidesPerView > 1) if (swiper.params.centeredSlides) swiper.visibleSlides.each((index, slide) => {
...
function translateTo(translate = 0, speed = this.params.speed, runCallbacks = true, translateBounds = true, internal) {
	...
	if (params.cssMode) {
		...
		else if (wrapperEl.scrollTo) wrapperEl.scrollTo({
...
function onScroll() {
	...
	swiper.previousTranslate = swiper.translate;
	if (swiper.isHorizontal()) if (rtlTranslate) swiper.translate = wrapperEl.scrollWidth - wrapperEl.offsetWidth - wrapperEl.scrollLeft;
...
function getBreakpoint(breakpoints) {
	...
	for (let i = 0; i < points.length; i += 1) {
		...
		if (value <= win.innerWidth) breakpoint = point;
	}
...
function addClasses() {
	const { classNames, params, rtl, $el } = this;
	const suffixes = [];
...
function loadImage(imageEl, src, srcset, sizes, checkForComplete, callback) {
	...
	if (!$(imageEl).parent("picture")[0] && (!imageEl.complete || !checkForComplete)) if (src) {
		image = new win.Image();
...
var Swiper = class Swiper extends SwiperClass {
	constructor(...args) {
		let el;
		let params;
	...
	slidesPerViewDynamic() {
		const { params, slides, slidesGrid, size: swiperSize, activeIndex } = this;
		let spv = 1;
		if (params.centeredSlides) {
...
var Keyboard = {
	handle(event) {
		...
		if (doc.activeElement && doc.activeElement.nodeName && (doc.activeElement.nodeName.toLowerCase() === "input" || doc.activeElement.nodeName.toLowerCase() === "textarea")) return;
		...
		if (!swiper.params.freeMode) {
			...
			if (!ignoreWheelEvents) {
				...
				if (swiper.params.freeModeSticky) {
					...
					if (!swiper.mousewheel.timeout) swiper.mousewheel.timeout = Utils.nextTick(() => {
...
var Mousewheel$1 = {
	...
	create() {
		const swiper = this;
		Utils.extend(swiper, { mousewheel: {
			enabled: false,
	...
	on: {
		...
		click(e) {
			...
			if (swiper.params.pagination.el && swiper.params.pagination.hideOnClick && swiper.pagination.$el.length > 0 && !$(e.target).hasClass(swiper.params.pagination.bulletClass)) {
				if (swiper.pagination.$el.hasClass(swiper.params.pagination.hiddenClass) === true) swiper.emit("paginationShow", swiper);
...
var Scrollbar = {
	...
	getPointerPosition(e) {
		if (this.isHorizontal()) return e.type === "touchstart" || e.type === "touchmove" ? e.targetTouches[0].clientX : e.clientX;
...
var Controller$1 = {
	...
	create() {
		const swiper = this;
		Utils.extend(swiper, { controller: {
...
var History = {
	...
	setHistory(key, index) {
		...
		if (swiper.params.history.replaceState) win.history.replaceState({ value }, null, value);
		else win.history.pushState({ value }, null, value);
	},
	slugify(text) {
...
var HashNavigation$1 = {
	...
	create() {
		const swiper = this;
		Utils.extend(swiper, { hashNavigation: {
			initialized: false,
...
var Autoplay$1 = {
	...
	on: {
		...
		sliderFirstMove() {
			const swiper = this;
			if (swiper.autoplay.running) if (swiper.params.autoplay.disableOnInteraction) swiper.autoplay.stop();
...
var EffectFade = {
	...
	create() {
		const swiper = this;
		Utils.extend(swiper, { fadeEffect: {
...
var EffectCube = {
	...
	create() {
		const swiper = this;
		Utils.extend(swiper, { cubeEffect: {
...
var EffectFlip = {
	...
	create() {
		const swiper = this;
		Utils.extend(swiper, { flipEffect: {
...
var EffectCoverflow = {
	...
	create() {
		const swiper = this;
		Utils.extend(swiper, { coverflowEffect: {
...
//# sourceMappingURL=swiper_js_swiper__esm__bundle__js.js.map